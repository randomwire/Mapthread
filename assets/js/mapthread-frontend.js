...
( function () {
	...
	const US_TIMEZONES =
		/^(America\/(New_York|Chicago|Denver|Los_Angeles|Phoenix|Anchorage|Adak|Boise|Indiana|Kentucky|Menominee|Nome|North_Dakota|Sitka|Yakutat|Juneau|Detroit|Metlakatla)|Pacific\/Honolulu|US\/)/;
	const useImperial = US_TIMEZONES.test(
	...
	const ICON_MINIMIZE =
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 13V19H13"/><path d="M5 5L19 19"/></svg>';
	const ICON_DOWNLOAD =
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 15V3"/><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><path d="m7 10 5 5 5-5"/></svg>';
	const ICON_LAYERS =
	...
	const ICON_INFO =
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>';
	 ...
	 * @return {[number, number]|null} Interpolated [lat, lng], or null if empty
	 */
	function interpolateAlongPath( coords, distances, totalDist, progress ) {
		...
		return [
			coords[ lo ][ 0 ] +
				t * ( coords[ lo + 1 ][ 0 ] - coords[ lo ][ 0 ] ),
			coords[ lo ][ 1 ] +
				t * ( coords[ lo + 1 ][ 1 ] - coords[ lo ][ 1 ] ),
		];
	}
	 ...
	 * @return {[number, number]|null} Latitude/longitude pair, or null if track empty
	 */
	 ...
	 *
	 * @param {number} progress - Fractional position along track (0 to 1)
	 * @return {Array<number>|null} [lat, lng] coordinate pair, or null
	 */
	 ...
	 *
	 * @param {number} progress - Fractional position along track (0 to 1)
	 * @return {Array<[number, number]>} Array of [lat, lng] coordinates
	 *
	 * @example
	 ...
	 * @return {number} Scroll progress from 0 (at marker top) to 1 (at next marker top)
	 *
	 * @description
	 * Special cases:
	...
	function toggleMapDismiss( btn ) {
		...
		document.body.classList.toggle(
			'mapthread-map-dismissed',
			isMapDismissed
		);

		const mapEl = map
			? map.getContainer()
			: document.getElementById( 'mapthread-map' );
	...
	function calculateElevationGainLoss( elevations ) {
		...
		for ( let i = 0; i < valid.length; i++ ) {
			const neighborhood = [];
			for (
				let j = Math.max( 0, i - mHalf );
				j <= Math.min( valid.length - 1, i + mHalf );
				j++
			) {
			...
			despiked[ i ] =
				neighborhood[ Math.floor( neighborhood.length / 2 ) ];
		}
		...
		for ( let i = 0; i < despiked.length; i++ ) {
			...
			for (
				...
				j <= Math.min( despiked.length - 1, i + half );
				j++
			) {
				sum += despiked[ j ];
				count++;
		...
		afterDraw( chart ) {
			...
			if ( markerTrackPositions.length > 0 ) {
				markerTrackPositions.forEach( ( pos, index ) => {
					...
					if (
						index === 0 ||
						index === markerTrackPositions.length - 1
					) {
						return;
					}
			...
			if ( showProgressIndicator && lastSmoothedProgress !== null ) {
				const progressKm =
					( lastSmoothedProgress * totalTrackDistance ) / 1000;
			...
			if ( trackHoverProgress !== null ) {
				const hoverKm =
					( trackHoverProgress * totalTrackDistance ) / 1000;
				...
				if ( hx >= chartArea.left && hx <= chartArea.right ) {
					...
					const hoverEle =
						getElevationAtProgress( trackHoverProgress );
					if ( hoverEle !== null ) {
				...
				scales: {
					...
					y: {
						...
						ticks: {
							...
							maxTicksLimit: 4,
							callback: ( value ) =>
								`${ Math.round( value ) } ${ elevationUnit() }`,
						},
						grid: {
				...
				plugins: {
					...
					tooltip: {
						...
						callbacks: {
							title: ( items ) => {
								if ( items.length > 0 ) {
									return `${ items[ 0 ].parsed.x.toFixed(
										1
									) } ${ distanceUnit() }`;
								}
								return '';
	...
	function initElevationProfile() {
		...
		const validElevations = trackElevations.filter(
			( e ) => e !== null && ! isNaN( e )
		);
		const maxEle =
			validElevations.length > 0
				? Math.round( Math.max( ...validElevations ) )
				: 0;
		const minEle =
			validElevations.length > 0
				? Math.round( Math.min( ...validElevations ) )
				: 0;
		// Build stats bar
		...
		statsBar.innerHTML =
			...
			'<span class="mapthread-stat-value">' +
			...
			'<span class="mapthread-stat-value">' +
			formatElevation( minEle ) +
			'</span>' +
			'</span>' +
			'<span class="mapthread-stat mapthread-stat-secondary">' +
			'<span class="mapthread-stat-label">Distance:</span> ' +
			'<span class="mapthread-stat-value">' +
			formatDistance( totalTrackDistance / 1000 ) +
			'</span>' +
			'</span>';
		...
		// Build chart data
		const distancesDisplay = trackDistances.map( ( d ) =>
			convertDistance( d / 1000 )
		);
		const elevationsDisplay = trackElevations.map( ( e ) =>
			e !== null && ! isNaN( e ) ? convertElevation( e ) : e
		);
	...
	function getMarkerElements() {
		if ( ! cachedMarkerElements ) {
			cachedMarkerElements =
				document.querySelectorAll( '.mapthread-marker' );
		}
	...
	function updateMapView( markerIndex ) {
		...
		map.flyTo( [ markerData.lat, markerData.lng ], markerData.zoom, {
	...
	function handleMarkerFollowScroll( activeMarker ) {
		...
		} else if ( activeMarker !== activeMarkerIndex ) {
	...
	const DismissControl = L.Control.extend( {
		...
		onAdd() {
			const container = L.DomUtil.create(
				'div',
				'leaflet-bar leaflet-control mapthread-dismiss-control'
			);
			...
			const btn = createControlBtn(
				container,
				'mapthread-dismiss-btn',
				initialIcon,
				initialLabel
			);
	...
	const DownloadControl = L.Control.extend( {
		...
		onAdd() {
			const container = L.DomUtil.create(
				'div',
				'leaflet-bar leaflet-control mapthread-download-control'
			);
			const btn = createControlBtn(
				container,
				'mapthread-download-btn',
				ICON_DOWNLOAD,
				'Download GPX'
			);
			...
			L.DomEvent.on( btn, 'click', ( e ) => {
				...
				const filename =
					gpxUrl.split( '/' ).pop().split( '?' )[ 0 ] || 'route.gpx';
	...
	const LayersControl = L.Control.extend( {
		...
		onAdd( mapInstance ) {
			...
			const container = L.DomUtil.create(
				'div',
				'leaflet-bar leaflet-control mapthread-layer-control'
			);

			const btn = ( this._btn = createControlBtn(
				container,
				'mapthread-layer-btn',
				ICON_LAYERS,
				'Map layers'
		...
		_switchLayer( name ) {
			...
			if ( this._current ) {
				this._map.removeLayer( this._current );
			}
	...
	const AttributionControl = L.Control.extend( {
		...
		onAdd( mapInstance ) {
			...
			const container = L.DomUtil.create(
				'div',
				'leaflet-bar leaflet-control mapthread-attribution-control'
			);
		...
		_updatePanel() {
			...
			const attrib = this._currentLayer
				? this._currentLayer.options.attribution || ''
				: '';
			...
			let imagery = attrib
				.replace(
					/,?\s*(?:Data\s+)?&copy;\s*<a[^>]*>OpenStreetMap<\/a>\s*contributors?/gi,
					''
				)
				.replace( /[,|]\s*/g, '' )
				.trim();
			...
			if ( hasOSM ) {
				html +=
					'<div class="mapthread-attribution-section">' +
				...
				html +=
					'<div class="mapthread-attribution-section">' +
					'<div class="mapthread-attribution-label">Map Data</div>' +
					'<div class="mapthread-attribution-value">' +
					attrib +
					'</div>' +
					'</div>';
			...
			// Footer — always shown
			html +=
				'<div class="mapthread-attribution-footer">' +
	...
	function initializeMap( bounds ) {
		...
		if ( gpxBlock && gpxBlock.parentNode ) {
			gpxBlock.parentNode.insertBefore(
				mapContainer,
				gpxBlock.nextSibling
			);
		} else {
		...
		L.control
			.zoom( {
				position: 'topright',
				zoomInText:
					'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12h14"/><path d="M12 5v14"/></svg>',
				zoomOutText:
					'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12h14"/></svg>',
			} )
			.addTo( leafletMap );
		...
		mapContainer.addEventListener(
			...
			( e ) => {
				...
				} else if ( ! scrollHintShown ) {
					scrollHintShown = true;
					scrollHint.classList.add(
						'mapthread-scroll-hint--visible'
					);
					setTimeout( () => {
		...
		const baseLayers = {};
		const layersConfig =
			typeof mapthreadConfig !== 'undefined' && mapthreadConfig.layers
				? mapthreadConfig.layers
				: {};
		...
		Object.keys( layersConfig ).forEach( ( providerId ) => {
			...
			if (
				! provider.apiKey ||
				! provider.styles ||
				! provider.styles.length
			) {
				return;
			}
			provider.styles.forEach( ( styleSlug ) => {
				const styleLabel =
					( provider.styleLabels &&
						provider.styleLabels[ styleSlug ] ) ||
					styleSlug;
				const layerName = provider.label + ' ' + styleLabel;
				const url = provider.url
					.replace( '{style}', styleSlug )
		...
		const attributionCtrl = new AttributionControl( {
			position: 'topleft',
		} );
		...
		fullscreenCtrl
			.getContainer()
			.classList.add( 'mapthread-fullscreen-control' );
		...
		const fullscreenBtn = fullscreenCtrl
			.getContainer()
			.querySelector( '.leaflet-fullscreen-icon' );
		if ( fullscreenBtn ) {
	...
	function addMarkerPins() {
		...
		markerElements.forEach( ( element, index ) => {
			try {
				...
				console.error(
					`Failed to create marker pin ${ index }:`,
					error
				);
		...
		if ( gpxBlock ) {
			...
			if ( gpxUrl ) {
				...
				if ( coords.length > 0 ) {
					...
					if (
						trackElevations.length === 0 &&
						trackCoords.length > 0
					) {
						try {
							const fetchedElevations =
								await fetchElevationFromAPI(
									attachmentId,
									trackCoords
								);
							...
							console.warn(
								'Mapthread: Failed to fetch elevation data',
								error
							);
